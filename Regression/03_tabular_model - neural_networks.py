...
def build_model(input_size=757, dropout_rate=0.2):
    ...
    model.add(
        tf.keras.layers.Dense(units=512, activation="mish", input_shape=(input_size,))
    )  # Input layer
    model.add(Dropout(dropout_rate))
    model.add(
        tf.keras.layers.Dense(
            units=256, activation="mish", kernel_constraint=MaxNorm(5)
        )
    )  # Hidden layer 1
    model.add(Dropout(dropout_rate))
    model.add(
        tf.keras.layers.Dense(
            units=128, activation="mish", kernel_constraint=MaxNorm(5)
        )
    )  # Hidden layer 2
    model.add(Dropout(dropout_rate))
    model.add(
        tf.keras.layers.Dense(units=64, activation="mish", kernel_constraint=MaxNorm(5))
    )  # Hidden layer 3
    model.add(Dropout(dropout_rate))
    model.add(
        tf.keras.layers.Dense(units=32, activation="mish", kernel_constraint=MaxNorm(5))
    )  # Hidden layer 4
    model.add(Dropout(dropout_rate))
    model.add(
        tf.keras.layers.Dense(units=16, activation="mish", kernel_constraint=MaxNorm(5))
    )  # Hidden layer 5
